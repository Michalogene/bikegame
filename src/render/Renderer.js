...
export class RendererSystem {
  ...
  constructor(root) {
    ...
    this.scene.fog = new THREE.FogExp2(0x0a1720, 0.0105);
    ...
    this.hemisphere = new THREE.HemisphereLight(0x7796b4, 0x1c281e, 1.18);
    ...
    this.fillLight = new THREE.DirectionalLight(0x3d6286, 0.72);
  ...
  updateLighting(lighting) {
    ...
    this.hemisphere.intensity = lerp(0.68, 1.44, daylight) * (1 - storm * 0.24);
    this.hemisphere.color.setRGB(
      lerp(0.34, 0.67, daylight),
      lerp(0.47, 0.79, daylight),
      lerp(0.64, 0.92, daylight)
    );
    this.hemisphere.groundColor.setRGB(0.09, lerp(0.12, 0.19, daylight), 0.085);
    this.keyLight.intensity = lerp(2.35, 3.3, daylight) * (1 - storm * 0.38);
    this.keyLight.color.setRGB(
      lerp(0.53, 1.0, daylight),
      lerp(0.66, 0.87, daylight),
      lerp(0.88, 0.69, daylight)
    );
    this.fillLight.intensity = 0.36 + night * 0.56;
    const fogColor = new THREE.Color().setRGB(
      lerp(0.028, 0.26, daylight),
      lerp(0.058, 0.34, daylight),
      lerp(0.082, 0.38, daylight)
    );
    this.scene.background.copy(fogColor);
    this.scene.fog.color.copy(fogColor);
    this.scene.fog.density = lerp(0.0102, 0.0058, daylight) + storm * 0.0055;
    this.renderer.toneMappingExposure = lerp(1.08, 1.08, daylight) - storm * 0.08;
  }
